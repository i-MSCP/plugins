...
if ( $action eq 'pre-configure' || $action eq 'pre-deconfigure' ) {
...
if ( $action eq 'pre-configure' ) {
    ...
    &_configureDovecot;
} elsif ( !grep($action eq $_, 'configure', 'pre-deconfigure') ) {
    die 'Unknown action';
} else {
    exit;
}
...
{
    iMSCP::Dir->new( dirname => '/etc/dovecot/imscp.d' )->make();

    my $file = iMSCP::File->new( filename => '/etc/dovecot/imscp.d/imscp_managesieve.conf' );
