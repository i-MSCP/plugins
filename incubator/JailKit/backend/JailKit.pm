    ...
    if (defined $main::execmode && $main::execmode eq 'setup') {
        ...
        $self->{'hooksManager'}->register('onBeforeAddImscpUnixUser', sub {
            ...
            if($parentUserGid && exists $rdata->{$customerId}) {
                my $customerName = $rdata->{$customerId}->{'jail_owner_name'};
...
sub run
{
    ...
    # Processing jaik items - Add/change/Remove jails

    my $rdata = $self->{'db'}->doQuery(
        'jail_id',
        "
            SELECT
                jail_id, jail_status, admin_name AS jail_owner_name, admin_sys_uid AS jail_owner_sys_uid
            FROM
                jailkit_jails
            INNER JOIN
                admin ON(admin_id = jail_owner_id)
    ...
    if(%{$rdata}) {
        ...
        for(keys %{$rdata}) {
            ...
            if($status ~~ ['toadd', 'tochange']) {
                $rs = $self->_addJail($rdata->{$_}->{'jail_owner_sys_uid'}, $rdata->{$_}->{'jail_owner_name'});

                @sql = (
                    'UPDATE jailkit_jails SET jail_status = ? WHERE jail_id = ?',
                    ($rs ? scalar getMessageByType('error') : 'ok'), $rdata->{$_}->{'jail_id'}
                );
            } elsif($status eq 'todelete') {
                $rs = $self->_deleteJail($rdata->{$_}->{'jail_id'}, $rdata->{$_}->{'jail_owner_name'});

                if($rs) {
                    @sql = (
                        'UPDATE jailkit_jails SET jail_status = ? WHERE jail_id = ?',
                        scalar getMessageByType('error'), $rdata->{$_}->{'jail_id'}
                    );
                } else {
                    @sql = ('DELETE FROM jailkit_jails WHERE jail_id = ?', $rdata->{$_}->{'jail_id'});
                }
            }
    ...
    # Processing SSH login items - Add/Change/Remove SSH users

    $rdata = $self->{'db'}->doQuery(
        'ssh_login_id',
        "
            SELECT
                ssh_login_id, ssh_login_name, ssh_login_pass, ssh_login_locked, ssh_login_status,
                admin_name AS jail_owner_name, admin_sys_uid AS jail_owner_sys_uid
            FROM
                jailkit_ssh_logins
            INNER JOIN
            ...
            INNER JOIN
                admin ON(admin_id = jail_owner_id)
            WHERE
                ssh_login_status IN('toadd', 'tochange', 'todelete')
            AND
                jail_status IN('ok', 'disabled')
        "
    );
    unless(ref $rdata eq 'HASH') {
        $self->{'FORCE_RETVAL'} = 'yes';
    ...
    if(%{$rdata}) {
        ...
        for(keys %{$rdata}) {
            my $status = $rdata->{$_}->{'ssh_login_status'};

            if($status eq 'toadd') {
                $rs = $self->_addSshUser(
                    $rdata->{$_}->{'jail_owner_sys_uid'}, $rdata->{$_}->{'jail_owner_name'},
                    $rdata->{$_}->{'ssh_login_name'}, $rdata->{$_}->{'ssh_login_pass'}
                );

                @sql = (
                    'UPDATE jailkit_ssh_logins SET ssh_login_status = ? WHERE ssh_login_id = ?',
                    ($rs ? scalar getMessageByType('error') : 'ok'), $rdata->{$_}->{'ssh_login_id'}
                );
            } elsif($status eq 'tochange') {
                $rs = $self->_changeSshUser(
                    $rdata->{$_}->{'jail_owner_sys_uid'}, $rdata->{$_}->{'jail_owner_name'},
                    $rdata->{$_}->{'ssh_login_name'}, $rdata->{$_}->{'ssh_login_pass'},
                    ($rdata->{$_}->{'ssh_login_locked'} eq '0')  ? 'unlock' : 'lock'
                );
                ...
                @sql = (
                    ...
                    $rdata->{$_}->{'ssh_login_id'}
                );
            } elsif($status eq 'todelete') {
                $rs = $self->_removeSshUser($rdata->{$_}->{'jail_owner_name'}, $rdata->{$_}->{'ssh_login_name'});

                if($rs) {
                    @sql = (
                        'UPDATE jailkit_ssh_logins SET ssh_login_status = ? WHERE ssh_login_id = ?',
                        scalar getMessageByType('error'), $rdata->{$_}->{'ssh_login_id'}
                    );
                } else {
                    @sql = (
                        'DELETE FROM jailkit_ssh_logins WHERE ssh_login_id = ?', $rdata->{$_}->{'ssh_login_id'}
                    );
                }
            }
...
sub uninstall
{
    ...
    my $rdata = $self->{'db'}->doQuery('ssh_login_name', 'SELECT ssh_login_name FROM jailkit_ssh_logins');
    unless(ref $rdata eq 'HASH') {
        error($rdata);
        return 1;
    }
    ...
    $rs = $self->{'db'}->doQuery('dummy', 'DROP TABLE IF EXISTS jailkit_jails');
    unless(ref $rs eq 'HASH') {
        error($rs);
        return 1;
    }
    ...
    $rs = $self->{'db'}->doQuery('dummy', 'DROP TABLE IF EXISTS jailkit_ssh_logins');
    unless(ref $rs eq 'HASH') {
        error($rs);
        return 1;
    }
...
sub _deleteJail($$$)
{
    ...
    my $rdata = $self->{'db'}->doQuery(
        'ssh_login_id', 'SELECT ssh_login_id, ssh_login_name FROM jailkit_ssh_logins WHERE jail_id = ?', $jailId
    );
    unless(ref $rdata eq 'HASH') {
        error($rdata);
        return 1;
    ...
    if(%{$rdata}) {
        ...
        for(keys %{$rdata}) {
            ...
            $rs = $self->{'db'}->doQuery(
                'dummy', 'DELETE FROM jailkit_ssh_logins WHERE ssh_login_id = ?', $rdata->{$_}->{'ssh_login_id'}
            );
            unless(ref $rs eq 'HASH') {
                error($rs);
                return 1;
...
sub _updateJails
{
    ...
    my $rdata = $self->{'db'}->doQuery(
        ...
        "
            SELECT
                jail_id, admin_name AS jail_owner_name, admin_sys_uid AS jail_owner_sys_uid
            FROM
                jailkit_jails
            INNER JOIN
                admin ON(admin_id = jail_owner_id)
    ...
    if(%{$rdata}) {
        ...
        for(keys %{$rdata}) {
            ...
            # it doesn't exist
            my $rs = $self->_addJail($rdata->{$_}->{'jail_owner_sys_uid'}, $rdata->{$_}->{'jail_owner_name'});
            return $rs if $rs;

            # Update the jail with last system files versions
            my ($stdout, $stderr);
            $rs = execute("jk_update -k -j $rootJailDir/$rdata->{$_}->{'jail_owner_name'}", \$stdout, \$stderr);
            debug($stdout) if $stdout;
            error($stderr) if $stderr && $rs;
            return $rs if $rs;
        }
...
sub _changeSshUsers($$)
{
    ...
    my $rdata = $self->{'db'}->doQuery(
        ...
        "
            SELECT
                ssh_login_id, ssh_login_name, ssh_login_pass, ssh_login_locked, admin_name AS jail_owner_name,
                admin_sys_uid AS jail_owner_sys_uid
            FROM
                jailkit_ssh_logins
            INNER JOIN
                jailkit_jails USING(jail_id)
            INNER JOIN
                admin ON(admin_id = jail_owner_id)
            WHERE
                ssh_login_status IN ('ok', 'disabled')
        "
    );
    unless(ref $rdata eq 'HASH') {
        error($rdata);
    ...
    if(%{$rdata}) {
        for(keys %{$rdata}) {
            ...
            my $jailKitLoginId = $rdata->{$_}->{'ssh_login_id'};
            my $rs = 0;

            unless(getpwnam($sshLoginName)) {
                # SSH user doesn't exist so we create it
                $rs = $self->_addSshUser(
                    $rdata->{$_}->{'jail_owner_sys_uid'}, $rdata->{$_}->{'jail_owner_name'}, $sshLoginName,
...
sub _processJkSocketdEntries
{
    ...
    if(%{$rdata}) {
        ...
        for(keys %{$rdata}) {
            $fileContent .= "[$rootJailDir/$rdata->{$_}->{'jail_owner_name'}/dev/log]\n";
