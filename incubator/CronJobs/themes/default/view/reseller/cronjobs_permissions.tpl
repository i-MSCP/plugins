...
<div id="page">
	<p class="hint" style="font-variant: small-caps;font-size: small;">
		This is the list of customers which are allowed to add cron jobs.
	</p>
...
<script>
	...
	function doRequest(rType, action, data) {
		return $.ajax({
			dataType: "json",
			type: rType,
			url: "/reseller/cron_permissions?action=" + action,
			data: data,
			timeout: 5000
		});
	}
	...
	$(document).ready(function () {
		...
		oTable = $(".datatable").dataTable({
			...
			bServerSide: true,
			"pagingType": "simple",
			sAjaxSource: "/reseller/cron_permissions?action=get_cron_permissions_list",
			bStateSave: true,
			aoColumnDefs: [
		...
		$("#admin_name").autocomplete({
			source: "/reseller/cron_permissions?action=search_customer",
			minLength: 2,
			delay: 500,
			autoFocus: true,
		...
		$("#page").on("click", "span[data-action],button", function (e) {
			...
			switch (action) {
				...
				case "delete_cron_permissions":
					if (confirm("Are you sure you want to revoke cron permissions for this customer?")) {
						doRequest(
