...
sub _init
{
    ...
    for ( qw/ PostfixMilterSocket clamav_milter_options clamav_unofficial_sigs_options / ) {
        exists $self->{'config'}->{$_} or die( sprintf( 'Missing %s configuration parameter', $_ ));
    }
...
{
    my $file = iMSCP::File->new( filename => "$main::imscpConfig{'PLUGINS_DIR'}/ClamAV/clamav-unofficial-sigs/clamav-unofficial-sigs.sh" );
...
sub _configureClamavUnofficialSigsConffile
{
    ...
    my $rs = iMSCP::File->new( filename => "$main::imscpConfig{'PLUGINS_DIR'}/ClamAV/clamav-unofficial-sigs/config/$confFile" )->copyFile(
...
sub _disableClamavUnofficialSigs
{
    ...
    my @files = ( '/etc/cron.d/clamav-unofficial-sigs', '/etc/logrotate.d/clamav-unofficial-sigs', '/usr/share/man/man8/clamav-unofficial-sigs.8' );
...
sub _installDistributionPackages
{
    ...
    $rs = execute(
        [
            ...
            '--assume-yes', '--auto-remove', '--no-install-recommends', '--purge', '--quiet', 'install', 'clamav', 'clamav-base', 'clamav-daemon',
            'clamav-freshclam', 'clamav-milter', 'curl', 'gnupg', 'rsync'
        ],
        \$stdout,
        \$stderr
    );
