...
class sauserprefs extends rcube_plugin
{
    ...
    function settings_tab($p)
    {
        ...
        $p['actions'][] = array(
            ...
            'class'    => 'sauserprefs',
            'label'    => 'sauserprefs.sauserprefs',
            'title'    => 'sauserprefs.managespam',
    ...
    function section_list($attrib)
    {
        ...
        $data = rcube::get_instance()->plugins->exec_hook(
            'sauserprefs_sections_list',
            array('list' => $this->sections, 'cols' => array('section'))
        );
    ...
    function gen_form($attrib)
    {
        $this->api->output->add_label(
            'sauserprefs.spamaddressexists', 'sauserprefs.spamenteraddress', 'sauserprefs.spamaddresserror',
            'sauserprefs.spamaddressdelete', 'sauserprefs.spamaddressdeleteall', 'sauserprefs.enabled',
            'sauserprefs.disabled', 'sauserprefs.importingaddresses', 'sauserprefs.usedefaultconfirm',
            'sauserprefs.purgebayesconfirm', 'sauserprefs.whitelist_from'
        );
    ...
    {
        $data = rcube::get_instance()->plugins->exec_hook(
            'sauserprefs_section_name',
            array(
                'section' => $this->cur_section,
                'title'   => $this->sections[$this->cur_section]['section']
            )
        );
        return $data['title'];
    }
    ...
    function save()
    {
        ...
        switch ($this->cur_section) {
            case 'general':
                if (!isset($no_override['required_score']))
                    $new_prefs['required_score'] = rcube_utils::get_input_value(
                        '_spamthres', rcube_utils::INPUT_POST
                    ) ?: $this->global_prefs['required_score'];

                if (!isset($no_override['rewrite_header Subject']))
                    $new_prefs['rewrite_header Subject'] = rcube_utils::get_input_value(
                        '_spamsubject', rcube_utils::INPUT_POST
                    );
                ...
                if (!isset($no_override['ok_locales'])) {
                    ...
                    if (is_array(rcube_utils::get_input_value('_spamlang', rcube_utils::INPUT_POST))) {
                        $locales = array_intersect(rcube_utils::get_input_value(
                            '_spamlang', rcube_utils::INPUT_POST), $this->sa_locales
                        );
                ...
                if (!isset($no_override['ok_languages']))
                    $new_prefs['ok_languages'] = is_array(
                        rcube_utils::get_input_value('_spamlang', rcube_utils::INPUT_POST)
                    ) ? implode(" ", rcube_utils::get_input_value('_spamlang', rcube_utils::INPUT_POST)) : '';
            ...
            case 'headers':
                ...
                if (!isset($no_override['add_header all Level'])) {
                    $spamchar = empty($_POST['_spamlevelchar'])
                        ? '*'
                        : rcube_utils::get_input_value('_spamlevelchar', rcube_utils::INPUT_POST);
            ...
            case 'bayes':
                ...
                if (!isset($no_override['bayes_auto_learn_threshold_nonspam']))
                    $new_prefs['bayes_auto_learn_threshold_nonspam'] = rcube_utils::get_input_value(
                        '_bayesnonspam', rcube_utils::INPUT_POST
                    ) ?: $this->global_prefs['bayes_auto_learn_threshold_nonspam'];

                if (!isset($no_override['bayes_auto_learn_threshold_spam']))
                    $new_prefs['bayes_auto_learn_threshold_spam'] = rcube_utils::get_input_value(
                        '_bayesspam', rcube_utils::INPUT_POST
                    ) ?: $this->global_prefs['bayes_auto_learn_threshold_spam'];
        ...
        if (!$data['abort']) {
            ...
            $this->api->output->command(
                'display_message', $data['message'] ? $data['message'] : $this->gettext('sauserpreffailed'), 'error'
            );
        }
    ...
    private function _load_prefs($user)
    {
        ...
        if (is_array($prefs['addresses']))
            usort(
                $prefs['addresses'],
                create_function('$a, $b', 'return strnatcasecmp($a["value"], $b["value"]);')
            );
    ...
    private function _prefs_block($part, $attrib)
    {
        ...
        switch ($part) {
            ...
            case 'general':
                $blocks = array(
                    'main'  => array(
                        'name'  => rcmail::Q($this->gettext('mainoptions')),
                        'class' => 'generalprefstable',
                        'cols'  => 2
                    ),
                    'langs' => array(
                        'name'  => rcmail::Q($this->gettext('langoptions')),
                        'class' => 'langprefstable',
                        'cols'  => 1
                    )
                );
                ...
                if (!isset($no_override['required_score'])) {
                    ...
                    if ($rcmail->config->get('sauserprefs_score_inc') - (int)$rcmail->config->get('sauserprefs_score_inc') > 0)
                        $decPlaces = strlen(
                                $rcmail->config->get('sauserprefs_score_inc') - (int)$rcmail->config->get('sauserprefs_score_inc')
                            ) - 2;

                    $score_found = false;
                    for ($i = 1; $i <= 10; $i = $i + $rcmail->config->get('sauserprefs_score_inc')) {
                        $input_spamthres->add(
                            number_format($i, $decPlaces, $locale_info['decimal_point'], ''),
                            number_format($i, $decPlaces, '.', '')
                        );

                        if (!$score_found && $this->user_prefs['required_score']
                            && (float)$this->user_prefs['required_score'] == (float)$i
                        )
                            $score_found = true;
                    }
                    ...
                    if (!$score_found && $this->user_prefs['required_score'])
                        $input_spamthres->add(
                            str_replace('%s', $this->user_prefs['required_score'], $this->gettext('otherscore')),
                            (float)$this->user_prefs['required_score']
                        );

                    $blocks['main']['options']['spamthres'] = array(
                        'title'   => html::label($field_id, rcmail::Q($this->gettext('spamthres'))),
                        'content' => $input_spamthres->show(
                            number_format($this->user_prefs['required_score'], $decPlaces, '.', '')
                        )
                    );
                ...
                if (!isset($no_override['rewrite_header Subject'])) {
                    ...
                    $input_spamsubject = new html_inputfield(array(
                        'name'  => '_spamsubject',
                        'id'    => $field_id,
                        'value' => $this->user_prefs['rewrite_header Subject'],
                        'style' => 'width:200px;'
                    ));
                ...
                if (!isset($no_override['ok_languages']) || !isset($no_override['ok_locales'])) {
                    $select_all = $this->api->output->button(array(
                        'command' => 'plugin.sauserprefs.select_all_langs',
                        'type'    => 'link',
                        'label'   => 'all'
                    ));
                    $select_none = $this->api->output->button(array(
                        'command' => 'plugin.sauserprefs.select_no_langs',
                        'type'    => 'link',
                        'label'   => 'none'
                    ));
                    $select_invert = $this->api->output->button(array(
                        'command' => 'plugin.sauserprefs.select_invert_langs',
                        'type'    => 'link',
                        'label'   => 'invert'
                    ));
                    ...
                    $blocks['langs']['options']['header'] = array(
                        ...
                        'content'         => $this->gettext('select') . ":&nbsp;&nbsp;" . $select_all . "&nbsp;&nbsp;"
                            . $select_invert . "&nbsp;&nbsp;" . $select_none
                    );

                    $lang_table = new html_table(array(
                        'id'          => 'spam-langs-table',
                        'class'       => 'records-table spam-langs-table fixedheader',
                        'cellspacing' => '0',
                        'cols'        => 2
                    ));
                    $lang_table->add_header(
                        'lang',
                        $this->api->output->button(array(
                            'command' => 'plugin.sauserprefs.table_sort',
                            'prop'    => '#spam-langs-table',
                            'type'    => 'link',
                            'label'   => 'language',
                            'title'   => 'sortby'
                        ))
                    ...
                    if (!isset($no_override['ok_languages'])) {
                        ...
                        $rcmail->config->set(
                            'sauserprefs_languages', array_intersect_key($tmp_array, array_flip($this->sa_locales))
                        );
                        $ok_languages = array();
                    }
                    ...
                    foreach ($rcmail->config->get('sauserprefs_languages') as $lang_code => $name) {
                        if (in_array($lang_code, $locales_langs))
                            $button = $this->api->output->button(array(
                                'command' => 'plugin.sauserprefs.message_lang',
                                'prop'    => $lang_code,
                                'type'    => 'link',
                                'class'   => 'enabled',
                                'id'      => 'spam_lang_' . $i,
                                'title'   => 'sauserprefs.enabled', 'content' => ' '
                            ));
                        else
                            $button = $this->api->output->button(array(
                                'command' => 'plugin.sauserprefs.message_lang',
                                ...
                                'class'   => 'disabled',
                                'id'      => 'spam_lang_' . $i,
                                'title'   => 'sauserprefs.disabled',
                                'content' => ' '
                            ));

                        $input_spamlang = new html_checkbox(array(
                            'style' => 'display: none;',
                            'name'  => '_spamlang[]',
                            'value' => $lang_code
                        ));
                        ...
                        $lang_table->add('tick', $button . $input_spamlang->show(
                                in_array($lang_code, $locales_langs) ? $lang_code : '')
                        );
            ...
            case 'headers':
                ...
                if (!isset($no_override['fold_headers'])) {
                    $help_button = html::img(array(
                        'class'  => 'helpicon', 'src' => $attrib['helpicon'],
                        'alt'    => $this->gettext('help'),
                        'border' => 0,
                        'style'  => 'margin-left: 4px;'
                    ));
                    $help_button = html::a(array(
                        'name'    => '_headerhlp',
                        'href'    => '#',
                        'onclick' => 'return ' . rcmail_output::JS_OBJECT_NAME . '.sauserprefs_help("fold_help");',
                        'title'   => $this->gettext('help')
                    ), $help_button);

                    $field_id = 'rcmfd_spamfoldheaders';
                    $input_spamreport = new html_checkbox(array(
                        'name' => '_spamfoldheaders',
                        'id'   => $field_id, 'value' => '1'
                    ));
                ...
                if (!isset($no_override['add_header all Level'])) {
                    ...
                    $help_button = html::a(array(
                        'name'    => '_headerhlp',
                        'href'    => '#',
                        'onclick' => 'return ' . rcmail_output::JS_OBJECT_NAME . '.sauserprefs_help("level_help");',
                        'title'   => $this->gettext('help')
                    ), $help_button);
            ...
            case 'tests':
                $blocks = array(
                    'main' => array('name'                      => rcmail::Q(
                        $this->gettext('mainoptions')), 'class' => 'testsprefstable', 'cols' => 3
                    )
                );
                ...
                if (!isset($no_override['use_razor1'])) {
                    ...
                    $help_button = html::a(array(
                        'name'    => '_headerhlp',
                        'href'    => '#',
                        'onclick' => 'return ' . rcmail_output::JS_OBJECT_NAME . '.sauserprefs_help("raz1_help");',
                        'title'   => $this->gettext('help')
                    ), $help_button);
                ...
                if (!isset($no_override['use_razor2'])) {
                    ...
                    $help_button = html::a(array(
                        'name'    => '_headerhlp',
                        'href'    => '#',
                        'onclick' => 'return ' . rcmail_output::JS_OBJECT_NAME . '.sauserprefs_help("raz2_help");',
                        'title'   => $this->gettext('help')
                    ), $help_button);
                ...
                if (!isset($no_override['use_pyzor'])) {
                    ...
                    $help_button = html::a(array(
                        'name'    => '_headerhlp',
                        'href'    => '#',
                        'onclick' => 'return ' . rcmail_output::JS_OBJECT_NAME . '.sauserprefs_help("pyz_help");',
                        'title'   => $this->gettext('help')
                    ), $help_button);
                ...
                if (!isset($no_override['use_dcc'])) {
                    ...
                    $help_button = html::a(array(
                        'name'    => '_headerhlp',
                        'href'    => '#',
                        'onclick' => 'return ' . rcmail_output::JS_OBJECT_NAME . '.sauserprefs_help("dcc_help");',
                        'title'   => $this->gettext('help')
                    ), $help_button);
                ...
                if (!isset($no_override['skip_rbl_checks'])) {
                    ...
                    $help_button = html::a(array(
                        'name'    => '_headerhlp',
                        'href'    => '#',
                        'onclick' => 'return ' . rcmail_output::JS_OBJECT_NAME . '.sauserprefs_help("rbl_help");',
                        'title'   => $this->gettext('help')
                    ), $help_button);
            ...
            case 'bayes':
                $blocks = array(
                    'main'      => array(
                        'name'  => rcmail::Q($this->gettext('mainoptions')),
                        'class' => 'bayesprefstable',
                        'cols'  => 3
                    ),
                    'autolearn' => array(
                        'name'  => rcmail::Q($this->gettext('bayesautooptions')),
                        'class' => 'bayesprefstable',
                        'cols'  => 2
                    )
                );
                ...
                if (!isset($no_override['use_bayes'])) {
                    //$help_button = html::img(array('class' => 'helpicon', 'src' => $attrib['helpicon'], 'alt' => $this->gettext('help'), 'border' => 0, 'style' => 'margin-left: 4px;'));
                    ...
                    if (!empty($this->bayes_query))
                        $delete_link = "&nbsp;&nbsp;&nbsp;" . html::span(
                                array('id' => 'listcontrols'),
                                $this->api->output->button(array(
                                    'command' => 'plugin.sauserprefs.purge_bayes',
                                    'type'    => 'link',
                                    'label'   => 'sauserprefs.purgebayes',
                                    'title'   => 'sauserprefs.purgebayesexp'
                                ))
                            );
                    else
                        $delete_link = '';
                ...
                if (!isset($no_override['use_bayes_rules'])) {
                    $help_button = html::img(array(
                        ...
                        'src'    => $attrib['helpicon'],
                        'alt'    => $this->gettext('help'),
                        'border' => 0, 'style' => 'margin-left: 4px;'
                    ));
                    $help_button = html::a(array(
                        'name'    => '_headerhlp',
                        'href'    => '#',
                        'onclick' => 'return ' . rcmail_output::JS_OBJECT_NAME . '.sauserprefs_help("bayesrules_help");',
                        'title'   => $this->gettext('help')
                    ), $help_button);

                    $field_id = 'rcmfd_spambayesrules';
                    $input_spamtest = new html_checkbox(array(
                        'name'     => '_spambayesrules',
                        'id'       => $field_id,
                        'value'    => '1',
                ...
                if (!isset($no_override['bayes_auto_learn'])) {
                    ...
                    $help_button = html::a(array(
                        'name'    => '_headerhlp',
                        'href'    => '#',
                        'onclick' => 'return ' . rcmail_output::JS_OBJECT_NAME . '.sauserprefs_help("bayesauto_help");',
                        'title'   => $this->gettext('help')
                    ), $help_button);
                ...
                if (!isset($no_override['bayes_auto_learn_threshold_nonspam'])) {
                    ...
                    $input_bayesnthres = new html_select(array(
                        'name'     => '_bayesnonspam',
                        'id'       => $field_id,
                        'disabled' => (!$this->user_prefs['bayes_auto_learn'] || !$this->user_prefs['use_bayes'])
                            ? 1 : 0
                    ));
                    ...
                    for ($i = -1; $i <= 1; $i = $i + 0.1) {
                        ...
                        if (!$score_found
                            ...
                            && (float)$this->user_prefs['bayes_auto_learn_threshold_nonspam'] == (float)$i
                        )
                            $score_found = true;
                    }
                    ...
                    if (!$score_found && $this->user_prefs['bayes_auto_learn_threshold_nonspam'])
                        $input_bayesnthres->add(
                            str_replace(
                                '%s', $this->user_prefs['bayes_auto_learn_threshold_nonspam'], $this->gettext('otherscore')
                            ),
                    ...
                    $blocks['autolearn']['options']['bayesnonspam'] = array(
                        ...
                        'content' => $input_bayesnthres->show(
                            number_format($this->user_prefs['bayes_auto_learn_threshold_nonspam'], $decPlaces, '.', '')
                        )
                    );
                ...
                if (!isset($no_override['bayes_auto_learn_threshold_spam'])) {
                    ...
                    $input_bayesthres = new html_select(array(
                        'name'     => '_bayesspam',
                        'id'       => $field_id,
                        'disabled' => (!$this->user_prefs['bayes_auto_learn'] || !$this->user_prefs['use_bayes'])
                            ? 1 : 0
                    ...
                    for ($i = 1; $i <= 20; $i = $i + $rcmail->config->get('sauserprefs_score_inc')) {
                        $input_bayesthres->add(
                            number_format(
                                $i, $decPlaces, $locale_info['decimal_point'], ''
                            ),
                            number_format($i, $decPlaces, '.', '')
                        );
                        ...
                        if (!$score_found
                            ...
                            && (float)$this->user_prefs['bayes_auto_learn_threshold_spam'] == (float)$i
                        )
                            $score_found = true;
                    }
            ...
            case 'report':
                ...
                if (!isset($no_override['report_safe'])) {
                    ...
                    $input_spamreport0 = new html_radiobutton(array(
                        'name'  => '_spamreport',
                        'id'    => $field_id . '_0',
                        'value' => '0'
                    ));
                    ...
                    $input_spamreport1 = new html_radiobutton(array(
                        'name'  => '_spamreport',
                        'id'    => $field_id . '_1',
                        'value' => '1'
                    ));
                    ...
                    $input_spamreport2 = new html_radiobutton(array(
                        'name'  => '_spamreport',
                        'id'    => $field_id . '_2',
                        'value' => '2'
                    ));
            ...
            case 'addresses':
                ...
                $input_spamaddressrule = new html_select(array(
                    'name' => '_spamaddressrule',
                    'id'   => $field_id
                ));
                ...
                $input_spamaddress = new html_inputfield(array(
                    'name'  => '_spamaddress',
                    'id'    => $field_id,
                    'style' => 'width:200px;'
                ));
                ...
                $button_addaddress = $this->api->output->button(array(
                    'command' => 'plugin.sauserprefs.addressrule_add',
                    'type'    => 'input',
                    'class'   => 'button',
                    'label'   => 'sauserprefs.addrule'
                ));
                ...
                $import = sizeof($this->addressbook_import) > 0 ?
                    $this->api->output->button(array(
                        'command' => 'plugin.sauserprefs.import_whitelist',
                        'type'    => 'link',
                        'label'   => 'import',
                        'title'   => 'sauserprefs.importfromaddressbook'
                    )) : '';
                $delete_all = $this->api->output->button(array(
                    'command' => 'plugin.sauserprefs.whitelist_delete_all',
                    'type'    => 'link',
                    'label'   => 'sauserprefs.deleteall'
                ));
                ...
                $address_table = new html_table(array(
                    ...
                    'class'       => 'records-table address-rules-table fixedheader',
                    'cellspacing' => '0',
                    'cols'        => 3
                ));
                $address_table->add_header(
                    'rule',
                    $this->api->output->button(array('command' => 'plugin.sauserprefs.table_sort',
                                                     'prop'    => '#address-rules-table',
                                                     'type'    => 'link',
                                                     'label'   => 'sauserprefs.rule',
                                                     'title'   => 'sortby')
                    ));
                $address_table->add_header(
                    'email',
                ...
                $table->add(
                    ...
                    html::div(array('id' => 'address-rules-cont'), $address_table->show())
                );
        ...
        foreach ($data['blocks'] as $block) {
            if (isset($block['content']) || sizeof($block['options']) > 0) {
                ...
                $out .= html::tag(
                    'fieldset', NULL, html::tag('legend', NULL, $block['name']) . $block['intro'] . $content
                );
            }
        }
    ...
    private function _address_row($address_table, $field, $value, $attrib)
    {
        ...
        $del_button = $this->api->output->button(array(
            'command' => 'plugin.sauserprefs.addressrule_del',
            'type'    => 'link',
            'class'   => 'delete',
            'label'   => 'delete',
            'content' => ' ',
        ...
        $address_table->add(
            'control', $del_button . $hidden_action->show() . $hidden_field->show() . $hidden_text->show()
        );
