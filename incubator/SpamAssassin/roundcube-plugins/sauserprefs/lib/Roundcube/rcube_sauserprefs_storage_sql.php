            ...
            if ($preference == 'addresses') {
                foreach ($value as $idx => $address) {
                    if ($address['action'] == "DELETE") {
                        ...
                        if (!$result) {
                            rcube::write_log('errors', 'sauserprefs error: cannot delete address "' . $address['field'] . '" = "' . $address['value'] . '" for ' . $user_id);
                            break;
                        }
                        ...
                        if (!$result) {
                            rcube::write_log('errors', 'sauserprefs error: cannot insert address"' . $address['field'] . '" = "' . $address['value'] . '" for ' . $user_id);
                            break;
                        }
                    }
                }
